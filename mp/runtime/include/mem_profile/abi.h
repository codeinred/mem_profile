...
namespace mp {
class name_demangler {
    constexpr static size_t initial_buff_size = 16384;
  ...
  public:
    name_demangler()
      : name_buff((char*)mperf_malloc(initial_buff_size))
    ...
    /// until name_demangler is destroyed, whichever is sooner
    ///
    ...
    auto demangle(std::string_view symbol) -> std::string_view {
        ...
        char* result = abi::__cxa_demangle(symbol_buff.c_str(), name_buff, &buff_size, &status);
    ...
    /// Invokes name_demangler::demangle
    auto operator()(std::string_view symbol) -> std::string_view { return demangle(symbol); }

    ~name_demangler() { mperf_free(name_buff); }
};
} // namespace mp
