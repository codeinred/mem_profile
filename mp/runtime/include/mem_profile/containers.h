...
namespace mp {
    ...
    /// Simple PriorityQueue implementation. Supports moving elements out of the
    /// queue. Moving elements out of the queue is necessary in order to modify
    /// them.
    template <class T, class Cmp = std::less<T>>
    class PriorityQueue : private Cmp {
        std::vector<T> vec;
       ...
       public:
        ...
        [[nodiscard]] T const& top() const {
            if (vec.empty()) {
                throw std::out_of_range(
                    "Attempted to access top of queue, but queue was empty");
            }
        ...
        [[nodiscard]] T pop() {
            if (vec.empty()) {
                throw std::out_of_range(
                    "Attempted to pop element from empty queue");
            }
    ...
    class SVStore {
       ...
       private:
        ...
        /// availible space Used for constructing things like a PriorityQueue
        struct CmpBlockSpace {
        ...
        PriorityQueue<Block, CmpBlockSpace> block_queue;

       public:
        /// Creates an SVStore with a default block size of
        /// SVStore::DEFAULT_BLOCK_SIZE
        SVStore() = default;
        SVStore(SVStore const&) = delete;
        ...
        SVStore(size_t default_block_size) noexcept
    ...
    class OrdIDStore : private std::vector<std::string_view> {
       ...
       public:
        OrdIDStore()
          : Base(1, std::string_view {})
          , prev()
          , store() {}
        /// Creates an OrdIDStore with the given initial capacity
        OrdIDStore(size_t cap)
          : OrdIDStore() {
            Base::reserve(cap);
        }
        OrdIDStore(OrdIDStore const&) = delete;
        OrdIDStore(OrdIDStore&&) = default;
        using Base::begin;
        using Base::capacity;
        using Base::end;
        using Base::size;
        ...
        /// to memory inside the IDStore's internal store
        ///
        /// It is safe to use string_views obtained from the store even after
        /// adding objects to the store: adding a string_view to the store will
        /// not invalidate any existing string_views that came from the store,
        ...
        ///
        /// Always returns 0 for null string_views
        [[nodiscard]] StoreResult save(std::string_view key) {
            if (key.data() == nullptr) {
            ...
            if (prev == key) {
                return StoreResult {prev, size() - 1};
            }
            ...
            prev = key;
            Base::push_back(key);
            return StoreResult {key, size() - 1};
        }
        [[nodiscard]] StoreResult save(char const* c_str) {
            if (c_str == nullptr) {
        ...
        [[nodiscard]] std::vector<std::string_view> const& keys()
            const noexcept {
            return *this;
        }
    ...
    class UniqueIDStore : private std::vector<std::string_view> {
        using Base = std::vector<std::string_view>;
        SVStore store {};
       ...
       public:
        ...
        [[nodiscard]] StoreResult save(std::string_view key) {
            ...
            if (prev == key) {
                return StoreResult {prev, prev_id};
            }
