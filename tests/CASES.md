...
| `aligned_c_api`          | L1    | `posix_memalign` + `aligned_alloc` owned/freed by a dtor. Interposed on macOS via the dyld table; known gap on Linux (glibc's aligned allocators bypass the exported `memalign` hook).                            |
